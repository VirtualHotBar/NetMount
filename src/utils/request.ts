...
function shouldRetry(
  ...
  if (error instanceof AppError) {
    const retryableTypes = [ErrorType.NETWORK, ErrorType.TIMEOUT, ErrorType.API]
...
export async function robustFetch<T = unknown>(
  ...
  try {
    ...
    return {
      success: false,
      error: error instanceof AppError ? error : AppError.api(String(error), 'UNKNOWN_ERROR'),
      attempts,
      duration: Date.now() - startTime,
    }
  }
