...
function searchStorageInfo(v: string | undefined, displayType: boolean = false): StorageInfoType {
  for (const storageItem of storageInfoList) {
    if (!displayType) {
      if (
        (storageItem.description === v || storageItem.label === v || storageItem.type === v) &&
        !storageItem.displayType
      ) {
        return storageItem
      }
    } else {
      if (
        storageItem.description === v ||
        storageItem.label === v ||
        storageItem.displayType === v
      ) {
        return storageItem
      }
    }
