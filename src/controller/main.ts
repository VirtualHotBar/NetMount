...
async function reupOpenlistVersion() {
  ...
  while (attempts < MAX_RETRIES) {
    ...
    if (attempts < MAX_RETRIES) {
      console.log(
        `Primary version endpoint failed, trying fallback... (attempt ${attempts}/${MAX_RETRIES})`
      )
      ...
      try {
        ...
        if (publicSettings.data?.version) {
          ...
          console.log(
            'OpenList version retrieved via /api/public/settings:',
            publicSettings.data.version
          )
          return true
        }
      } catch (fallbackError) {
      ...
      if (attempts < MAX_RETRIES) {
        console.log(
          `All version endpoints failed, retrying in ${RETRY_DELAY_MS}ms... (attempt ${attempts + 1}/${MAX_RETRIES})`
        )
  ...
  // 所有重试都失败了
  console.error(
    'All version endpoints failed after',
    MAX_RETRIES,
    'attempts. Using fallback version.'
  )
