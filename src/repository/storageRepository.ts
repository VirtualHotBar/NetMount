...
export class StorageRepository extends EventEmitter {
  ...
  async refresh(): Promise<void> {
    try {
      ...
      const appError = AppError.api('Failed to refresh storage list', 'REFRESH_FAILED', {
        error: String(error),
      })
  ...
  search(criteria: StorageSearchCriteria): StorageList[] {
    ...
    if (criteria.name) {
      results = results.filter(s => s.name.toLowerCase().includes(criteria.name!.toLowerCase()))
    }
  ...
  getOpenListById(id: number): StorageList | undefined {
    return this.cache.find(s => s.framework === 'openlist' && s.other?.openlist?.id === id)
  }
...
export function validateStorageName(name: string): AppError | null {
  ...
  // 检查非法字符
  const invalidChars = STORAGE_CONSTANTS.INVALID_STORAGE_CHARS.filter(char => name.includes(char))
...
export function safeAddStorage(
  ...
  try {
    ...
    return {
      success: false,
      error: AppError.api('Failed to add storage', 'ADD_STORAGE_FAILED', {
